...
if ($_SERVER["REQUEST_METHOD"] != "POST"){
	header("Location: https://bioshiitake.tirol/?error=1&msg=noPost#contact");
	die();
}

if(empty($_POST["name"]) OR empty($_POST["email"]) OR empty($_POST["betreff"]) OR empty($_POST["nachricht"]) OR !empty($_POST["testfield"])){
	header("Location: https://bioshiitake.tirol/?error=1&msg=notAllSet#contact");
	die();
}
...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)){
	header("Location: https://bioshiitake.tirol/?error=1&msg=invalidEmail#contact");
	die();
}
...
try {
	...
	header("Location: https://bioshiitake.tirol/?error=0#contact");
	die();
} catch (Exception $e) {
	error_log("Message could not be sent. Mailer Error: {$mail->ErrorInfo}", 1, "admin@cafeshivani.at");
	header("Location: https://bioshiitake.tirol/?error=1#contact");
	die();
}
?>